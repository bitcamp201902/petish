...
function commentList(param) {
	...
	$.ajax({
		url:'/dog/missingboard/commentList',
		type:'GET',
		data : params,
...
$('#input_data').click(function(event){
	...
	var loginedUser = $('#loginUser').val();
	
	//alert(params);
	
	jQuery.ajax({
		url : '/dog/missingboard/insertComment',
		type : 'POST',
		data : params,
		contentType:'application/x-www-form-urlencoded; charset=UTF-8',
		dataType : "json",
...
function modifyComment(id, callback, error) {
	...
	$.ajax({
		type : 'put',
		url : '/dog/missingboard/modifyComment/' + id,
...
function removeComment(id) {
	...
	if(confirm("삭제하시겠습니까?")) {		
		$.ajax({
			type : 'put',
			url : '/dog/missingboard/removeComment/' + id,
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr) {
				if(result) {
					//alert('댓글삭제!');
